...
#include <lgv_nav350/SickNAV350.hh>
...
class sick_nav350: public rclcpp::Node
{
    public:
        ...
        {
            auto scan_pub = this->create_publisher<sensor_msgs::msg::LaserScan>("scan", rclcpp::QoS(rclcpp::SensorDataQoS()));
    ...
    private:
        ...
        geometry_msgs::TransformStamped sickn350_to_target_tf;
        geometry_msgs::TransformStamped target_to_mobile_base_tf;
    ...
    private:
        ...
        void publish_scan(rclcpp::Publisher<sensor_msgs::msg::LaserScan>::SharedPtr& pub, double *range_values,
                  ...
                  unsigned int sector_start_timestamp, rclcpp::Publisher<sicktoolbox_wrapper>::SharedPtr *pub1)         
...
void createOdometryMessage(const rclcpp::Duration& time_elapsed, const tf2_ros::Transform& prev_transform,
                           const tf2_ros::Transform& current_transform, nav_msgs::msg::Odometry& odom_msg)
{
...
{
  rclcpp::init(argc, argv, "sicknav350");
  ...
  try
  {
    ...
    else
    {
      try
      {
        ...
        if(!tf_listerner.waitForTransform(
            target_frame_id,sick_frame_id,rclcpp::Time(0),rclcpp::Duration(TRANSFORM_TIMEOUT),
            rclcpp::Duration(POLLING_DURATION),&error_msg))
        {
          RCLCPP_ERROR_STREAM("Transform lookup timed out, error msg: "<<error_msg);
          return -1;
        }
      ...
      {
        RCLCPP_ERROR_STREAM("Transform lookup between "<<sick_frame_id<<" and "<<target_frame_id<<" failed, exiting");
        return -1;
      }
    }
    ...
    tf2_ros::Transform mobile_base_current_tf = tf2_ros::Transform::getIdentity();
    tf2_ros::Transform mobile_base_prev_tf = tf2_ros::Transform::getIdentity();
    nav_msgs::msg::odometry odom_msg;

    if(publish_odom)
    {
      odom_pub = nh.advertise<nav_msgs::msg::odometry>(ODOM_TOPIC, 1);
      ...
      catch(tf2_ros::LookupException &exp)
      {
    ...
    while (rclcpp::ok())
    {
      ...
      tf2_ros::Transform odom_to_sick_tf;
      tf2_ros::Transform odom_to_target_tf;
      tf2_ros::Quaternion odomquat=tf2_ros::createQuaternionFromYaw(DEG2RAD(phi1/1000.0));
      odomquat.inverse();
      odom_to_sick_tf.setRotation(odomquat);
      odom_to_sick_tf.setOrigin(tf2_ros::Vector3(-x1 / 1000, -y1/ 1000, 0.0));
      ...
      odom_broadcasters.sendTransform(tf2_ros::StampedTransform(odom_to_target_tf, rclcpp::Clock().now(),
      ...
      RCLCPP_DEBUG_STREAM("NAV350 # reflectors seen:"<<number_reflectors);
      RCLCPP_DEBUG_STREAM("NAV350 # reflectors used:"<<num_reflectors);
      for (int r=0;r<number_reflectors; r++)
      {
        Rx[r]=(double) sick_nav350.ReflectorData_.x[r];
        Ry[r]=(double) sick_nav350.ReflectorData_.y[r];
        RCLCPP_DEBUG_STREAM("Reflector "<<r<<" x pos: "<<Rx[r]<<" y pos: "<<Ry[r]);
      }
