...
namespace SickToolbox {
  ...
  public:
    ...
    void GetSickSignals( uint8_t &sick_signal_flags ) noexcept(false);
  
    /** Enables nearfield suppressive filtering (in flash) */
    void EnableNearfieldSuppression( ) noexcept(false);

    /** Disables nearfield suppressive filtering (in flash) */
    void DisableNearfieldSuppression( ) noexcept(false);
  ...
  private:
    ...
    void _syncDriverWithSick( ) noexcept(false);
    ...
    void _setSickSensorModeToIdle( ) noexcept(false);

    /** Sets the Sick LD to ROTATE mode */
    void _setSickSensorModeToRotate( ) noexcept(false);

    /** Sets the Sick LD to MEASURE mode */
    void _setSickSensorModeToMeasure( ) noexcept(false);
    ...
    void _cancelSickScanProfiles( ) noexcept(false);
    ...
    void _getSickIdentity( ) noexcept(false);
    ...
    void _getSickGlobalConfig( ) noexcept(false);

    /** Query the Sick for its Ethernet configuration parameters */
    void _getSickEthernetConfig( ) noexcept(false);

    /** Acquires the configuration (function and stop angle) for each sector */
    void _getSickSectorConfig( ) noexcept(false);
    ...
    void _getSensorPartNumber( ) noexcept(false);

    /** Query the Sick for its assigned name */
    void _getSensorName( ) noexcept(false);

    /** Query the Sick for its version number */
    void _getSensorVersion( ) noexcept(false);

    /** Query the Sick for its serial number */
    void _getSensorSerialNumber( ) noexcept(false);

    /** Query the Sick for its EDM unit's serial number */
    void _getSensorEDMSerialNumber( ) noexcept(false);

    /** Query the Sick for the part number of its firmware */
    void _getFirmwarePartNumber( ) noexcept(false);

    /** Query the Sick for the name of its firmware */
    void _getFirmwareName( ) noexcept(false);

    /** Query the Sick for the version of the firmware */
    void _getFirmwareVersion( ) noexcept(false);

    /** Query the part number of the application software */
    void _getApplicationSoftwarePartNumber( ) noexcept(false);

    /** Query the Sick for the application name */
    void _getApplicationSoftwareName( ) noexcept(false);

    /** Query the Sick for the application software version */
    void _getApplicationSoftwareVersion( ) noexcept(false);
    ...
    void _flushTCPRecvBuffer( ) noexcept(false);
