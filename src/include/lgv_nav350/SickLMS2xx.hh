...
namespace SickToolbox {
  ...
  public:
    ...
    sick_lms_2xx_type_t GetSickType( ) const noexcept(false);

    /** Gets the scan angle currently being used by the device */
    double GetSickScanAngle( ) const noexcept(false);

    /** Gets the scan resolution currently being used by the device */
    double GetSickScanResolution( ) const noexcept(false);
    ...
    SickLMS2xx::sick_lms_2xx_measuring_units_t GetSickMeasuringUnits( ) const noexcept(false);
    ...
    sick_lms_2xx_sensitivity_t GetSickSensitivity( ) const noexcept(false);
    ...
    /** Get the current Sick LMS 2xx sensitivity level. NOTE: Only applies to LMS 211/221/291 models. */
    sick_lms_2xx_peak_threshold_t GetSickPeakThreshold( ) const noexcept(false);
    ...
    sick_lms_2xx_measuring_mode_t GetSickMeasuringMode( ) const noexcept(false);

    /** Get the current Sick LMS 2xx operating mode */
    sick_lms_2xx_operating_mode_t GetSickOperatingMode( ) const noexcept(false);
    ...
    uint8_t GetSickAvailability( ) const noexcept(false);
    ...
    void GetSickScan( unsigned int * const measurement_values,
		      ...
		      unsigned int * const sick_real_time_scan_index = NULL ) noexcept(false);
    ...
    void GetSickMeanValues( const uint8_t sick_sample_size,
			    ...
			    unsigned int & num_measurement_values,
			    unsigned int * const sick_telegram_index = NULL,
			    unsigned int * const sick_real_time_index = NULL ) noexcept(false);
    ...
    sick_lms_2xx_status_t GetSickStatus( ) noexcept(false);

    /** Indicates whether the Sick is an LMS Fast */
    bool IsSickLMS2xxFast( ) const noexcept(false);
  ...
  protected:
    ...
    void _setupConnection() noexcept(false);
    void _setupConnection(const uint32_t delay ) noexcept(false);

    /** Closes the serial communication terminal. */
    void _teardownConnection( ) noexcept(false);
    ...
    void _sendMessageAndGetReply( const SickLMS2xxMessage &sick_send_message,
				  ...
				  const unsigned int num_tries ) noexcept(false);
    ...
    void _setSessionBaud( const sick_lms_2xx_baud_t baud_rate ) noexcept(false);

    /** Tests communication wit the LMS at a particular baud rate. */
    bool _testSickBaud( const sick_lms_2xx_baud_t baud_rate ) noexcept(false);

    /** Changes the terminal's baud rate. */
    void _setTerminalBaud( const sick_lms_2xx_baud_t sick_baud ) noexcept(false);

    /** Gets the type of Sick LMS */
    void _getSickType( ) noexcept(false);

    /** Gets the current Sick configuration settings */
    void _getSickConfig( ) noexcept(false);

    /** Sets the Sick configuration in flash */
    void _setSickConfig( const sick_lms_2xx_device_config_t &sick_config ) noexcept(false);
    
    /** Gets the status of the LMS */
    void _getSickStatus( ) noexcept(false);
    ...
    void _getSickErrors( unsigned int * const num_sick_errors = NULL,
			 ...
			 uint8_t * const error_num_buffer = NULL ) noexcept(false);
