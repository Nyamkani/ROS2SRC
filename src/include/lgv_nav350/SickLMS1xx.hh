...
namespace SickToolbox {
  ...
  public:
    ...
    void Initialize( const bool disp_banner = true ) noexcept(false); /*throw( SickIOException, SickThreadException, SickTimeoutException, SickErrorException );*/
    ...
    void SetSickScanFreqAndRes( const sick_lms_1xx_scan_freq_t scan_freq,
				const sick_lms_1xx_scan_res_t scan_res ) noexcept(false); /*throw( SickTimeoutException, SickIOException, SickErrorException );*/ 

    /** Get the Sick LMS 1xx scan frequency */
    sick_lms_1xx_scan_freq_t GetSickScanFreq( ) const noexcept(false); /*throw ( SickIOException );*/
    
    /** Get the Sick LMS 1xx scan resolution */
    sick_lms_1xx_scan_res_t GetSickScanRes( ) const noexcept(false); /*throw ( SickIOException );*/

    /** Get the Sick LMS 1xx scan start angle */
    double GetSickStartAngle( ) const noexcept(false); /*throw ( SickIOException );*/

    /** Get the Sick LMS 1xx scan stop angle */
    double GetSickStopAngle( ) const noexcept(false); /*throw ( SickIOException );*/

    /** Sets the sick scan data format */
    void SetSickScanDataFormat( const sick_lms_1xx_scan_format_t scan_format ) noexcept(false); /*throw( SickTimeoutException, SickIOException, SickThreadException, SickErrorException );*/
    ...
    void GetSickMeasurements( unsigned int * const range_1_vals,
			      ...
			      unsigned int * const dev_status = NULL ) noexcept(false); /*throw ( SickIOException, SickConfigException, SickTimeoutException );*/

    /** Uninitializes the Sick LD unit */
    void Uninitialize( const bool disp_banner = true ) noexcept(false); /*throw( SickIOException, SickTimeoutException, SickErrorException, SickThreadException );*/
  ...
  private:
    ...
    void _setupConnection( ) noexcept(false); /*throw( SickIOException, SickTimeoutException );*/

    /** Re-initialized the device */
    void _reinitialize( ) noexcept(false); /*throw( SickIOException, SickThreadException, SickTimeoutException, SickErrorException );*/     

    /** Teardown the connection to the Sick LD */
    void _teardownConnection( ) noexcept(false); /*throw( SickIOException );*/

    /** Acquire the latest Sick LMS's status */
    void _updateSickStatus( ) noexcept(false); /*throw( SickTimeoutException, SickIOException );*/

    /** Acquire the Sick LMS's scan config */
    void _getSickScanConfig( ) noexcept(false); /*throw( SickTimeoutException, SickIOException );*/
    ...
    void _setSickScanConfig( const sick_lms_1xx_scan_freq_t scan_freq,
			     ...
			     const int start_angle, const int stop_angle ) noexcept(false); /*throw( SickTimeoutException, SickIOException, SickErrorException );*/
    
    /** Set access mode for configuring device */
    void _setAuthorizedClientAccessMode( ) noexcept(false); /*throw( SickTimeoutException, SickErrorException, SickIOException );*/

    /** Save configuration parameters to EEPROM */
    void _writeToEEPROM( ) noexcept(false); /*throw( SickTimeoutException, SickIOException );*/

    /** Send the message w/o waiting for a reply */
    void _sendMessage( const SickLMS1xxMessage &send_message ) const noexcept(false); /*throw ( SickIOException );*/
    ...
    void _sendMessageAndGetReply( const SickLMS1xxMessage &send_message,
				  ...
				  const unsigned int num_tries = 1 ) noexcept(false);/*throw( SickIOException, SickTimeoutException );*/

    /** Receive a message */
    void _recvMessage( SickLMS1xxMessage &sick_message ) const noexcept(false); /*throw ( SickTimeoutException );*/
    
    /** Start device measuring */
    void _startMeasuring( ) noexcept(false); /*throw ( SickTimeoutException, SickIOException );*/

    /** Stop device measuring */
    void _stopMeasuring( ) noexcept(false); /*throw ( SickTimeoutException, SickIOException );*/

    /** Request a data data stream type */
    void _requestDataStream( ) noexcept(false); /*throw ( SickTimeoutException, SickConfigException, SickIOException );*/
    
    /** Start streaming measurements */
    void _startStreamingMeasurements(  ) noexcept(false); /*throw( SickTimeoutException, SickIOException );*/

    /** Stop streaming measurements */
    void _stopStreamingMeasurements( const bool disp_banner = true ) noexcept(false); /*throw( SickTimeoutException, SickIOException );*/

    /** Set device to measuring mode */
    void _checkForMeasuringStatus( unsigned int timeout_value = DEFAULT_SICK_LMS_1XX_STATUS_TIMEOUT ) noexcept(false);

    /** Sets the sick scan data format */
    void _setSickScanDataFormat( const sick_lms_1xx_scan_format_t scan_format ) noexcept(false);
    
    /** Restore device to measuring mode */
    void _restoreMeasuringMode( ) noexcept(false);
